...
describe("ID persistence through replay", () => {
  it("IDs generated by ensureTodoIds survive replayFromBranch", () => {
    // Simulate: first write generates IDs
    const todosA = [
    ...
    expect(rA.ok).toBe(true);
    if (!rA.ok) return;
    const withIds = ensureTodoIds(rA.todos, getTodos());
    setTodos(withIds);
    const storedAfterA = getTodos();
    expect(storedAfterA[0].id).toBeDefined();
    expect(storedAfterA[1].id).toBeDefined();

    // Second write: update status, keep IDs
    const todosB = [
      { content: "Alpha", status: "completed", priority: "high", id: storedAfterA[0].id },
      { content: "Beta", status: "in_progress", priority: "medium", id: storedAfterA[1].id },
    ];
    const rB = validateTodoWrite(todosB, getTodos());
    expect(rB.ok).toBe(true);
    if (!rB.ok) return;
    ...
    expect(replayed[0].status).toBe("completed");
    expect(replayed[1].id).toBe(storedAfterA[1].id);
    expect(replayed[1].status).toBe("in_progress");
  });
});
