...
describe("prompt guidance", () => {
  it("TASK_MANAGEMENT_SECTION nudges cold-start todowrite for explain/explore", () => {
    expect(TASK_MANAGEMENT_SECTION).toContain("todowrite");
    expect(TASK_MANAGEMENT_SECTION).toMatch(/explain\/explore\/review/i);
    expect(TASK_MANAGEMENT_SECTION).toMatch(/FIRST tool call must be todowrite/i);
  ...
  it("COLD_START_BOOST is present for prompt-aware path", async () => {
    ...
    expect(COLD_START_BOOST).toContain("Call todowrite before any other tool");
  });
